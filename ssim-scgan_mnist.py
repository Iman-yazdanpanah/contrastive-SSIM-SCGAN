...
tf.app.flags.DEFINE_string(
    'result_dir', 'resultexpssimp1.0v1.5',
    'The directory to save result of scgan.')
tf.app.flags.DEFINE_string(
    'checkpoint_dir', 'checkpointexpssimp1.0v1.5',
...
result_dir = os.path.join(FLAGS.result_dir, 'scgan_' + FLAGS.dataset_type)
checkpoint_dir = os.path.join(FLAGS.checkpoint_dir, 'scgan_' + FLAGS.dataset_type)
...
if FLAGS.train:
    ...
    for i in r:
        for j in r1:
            if i<j:
                a = tf.reduce_sum(c_one_hot[i]*c_one_hot[j])
                sim =1.0-tf.reduce_mean(tf.image.ssim(G_fake[i],G_fake[j], 1))
                sim_reg_list.append(a*tf.exp(1.0*sim)+(1.-a)*(tf.exp(-sim+1.5)))
                ti=ti+1
...
def test():
    ...
    np.save(os.path.join(result_dir , 'ssim-scgan_' + FLAGS.dataset_type + '.npy'), 
