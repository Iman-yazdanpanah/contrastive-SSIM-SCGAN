...
tf.app.flags.DEFINE_boolean(
    'train', False, 'Whether to train or test.')
...
tf.app.flags.DEFINE_string(
    'data_dir', './',
    'The directory to load dataset cifar10.')
tf.app.flags.DEFINE_string(
    'result_dir', 'resultexptssimp1.0v1.5',
    'The directory to save result of scgan.')
tf.app.flags.DEFINE_string(
    'checkpoint_dir', 'checkpointexptssimp1.0v1.5',
    'The directory to save or load checkpoint file.')
tf.app.flags.DEFINE_string(
    'dataset_type', 'cifar10', 'The dataset cifar10.')
...
if FLAGS.train:
    ...
    for i in r:
        for j in r1:
            if i<j:
                a = 1. - tf.abs(c[i]-c[j])
                sim =1.0-tf.reduce_mean(tf.image.ssim(G_fake[i],G_fake[j], 1))
                sim_reg_list.append(tf.reduce_sum(a*tf.exp(1.0*sim)+(1.-a)*(tf.exp(-sim+1.5))))
                ti=ti+1
    sim_reg = tf.truediv(tf.add_n(sim_reg_list), ti)
...
def get_inception_score(sess):
    ...
    fakes = np.concatenate(fake_list, axis=0)
    fakes = (fakes+1.)*(255./2.)  # change to range [0, 255]
    return inception_score.get_inception_score(list(fakes))
    ...
    with tf.Session() as sess:
        ...
        for i in range(FLAGS.iters):
            ...
            if i % 1000 == 999:
                ...
                inception_mean, inception_std = get_inception_score(sess)
                inception_list.append(inception_mean)
                inception_step.append(i)
                utils.save_plot(inception_step, inception_list, 
                                os.path.join(result_dir, 'inception_score.jpg'),
                                title_name='SCGAN on ' + FLAGS.dataset_type.upper(), y_label_name='inception_score')
...
def test():
    ...
    # ckpt = 'E:/python_workspace/paper-scgan-copy/checkpoint/scgan_cifar10_continuous/model.ckpt-300000'
    with tf.Session() as sess:
        saver.restore(sess, ckpt)
        test_z = np.random.normal(size=[32, FLAGS.z_dim])
        test_c = np.zeros([32, 2])
        test_c[:, 1-FLAGS.con_dim] = np.random.uniform() + np.random.normal(0.0, 0.001, 32)
        for i in range(10):
    ...
    utils.save_images(images[...,::-1], [32, 10], 'cifar_c%d.png' % FLAGS.con_dim, norm='[-1,1]')
    np.save('cifar_c%d.npy' % FLAGS.con_dim, images)
