...
tf.app.flags.DEFINE_integer(
    'con_dim', 2, 'The index of variational conditional variable for test, 0, 1, 2 or 3.')
    ...
    for i in r:
        for j in r1:
            if i<j:
              ...
              sim_reg_list.append(a_0*tf.exp(1.0*sim)+(1.-a_0)*(tf.exp(-sim+1.5)))          
              sim_reg_list.append(a_1*tf.exp(1.0*sim)+(1.-a_1)*(tf.exp(-sim+1.5)))
              sim_reg_list.append(a_2*tf.exp(1.0*sim)+(1.-a_2)*(tf.exp(-sim+1.5)))
              sim_reg_list.append(a_3*tf.exp(1.0*sim)+(1.-a_3)*(tf.exp(-sim+1.5)))
              ti=ti+1
